...
const SHEET_URL = "https://script.google.com/macros/s/AKfycbwmS_CXZONWiNtOkAqu-p4Y56gMOy9t07mYJCk-L3Z_oV7ohV_GyYSZwKZbxwUvTFf2PQ/exec";
...
async function syncToSheet(todayLog, score, date) {
  try {
    await fetch(SHEET_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ log: todayLog, score: score || "", date }),
    });
  } catch(e) {
...
async function analyzeFood(text, image, targets, todayNutrients, meal = "breakfast") {
  ...
  const systemPrompt = `你是一位專業的營養師助理。請分析用戶輸入的食物，回傳 JSON 格式：
{
  ...
  "advice": "${nextMeal}建議（繁體中文，3條具體建議，每條需包含：目前該營養素佔每日目標百分比、具體補充量和食物建議。格式：• 蛋白質補強：目前僅佔每日目標X%，建議${nextMeal}攝取...）"
}
只回傳 JSON，不要其他文字。`;
...
export default function HealthLog() {
  ...
  const [recordInput, setRecordInput] = useState({ text: "", meal: "breakfast", image: null });
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        setRecordInput(r => ({ ...r, image: { data: compressed, type: "image/jpeg" } }));
      };
  ...
  const handleRecord = async () => {
    if (!recordInput.text.trim() && !recordInput.image) return;
    setRecordState("loading");
    setLoadingStep(0);
    const nutrients = getTodayNutrients();
    const result = await analyzeFood(recordInput.text, recordInput.image, profile.targets, nutrients, recordInput.meal);
  ...
  const handleConfirm = () => {
    ...
    setRecordInput({ text: "", meal: "breakfast", image: null });
    ...
    <div>
      ...
      {lastAdvice && (
        <div style={{ ...styles.card, background: COLORS.greenPale, border: `1px solid ${COLORS.greenLight}` }}>
          ...
          <div style={{ fontSize: 14, color: COLORS.text, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>
            {lastAdvice.text}
          </div>
        </div>
      )}
        ...
        <>
          <div style={styles.card}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button style={{ ...styles.btnOutline, flex: 1 }} onClick={() => fileRef.current?.click()}>
                📷 {recordInput.image ? "已上傳圖片 ✓" : "上傳照片"}
              </button>
          ...
          <div style={{ margin: "0 16px" }}>
            <button style={styles.btn} onClick={handleRecord}
              disabled={!recordInput.text.trim() && !recordInput.image}>
              AI 分析 →
            </button>
          </div>
        </>
        ...
        <>
          ...
          {recordResult.advice && (
            <div style={styles.card}>
              ...
              <div style={{ fontSize: 14, color: COLORS.text, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{recordResult.advice}</div>
            </div>
          )}
